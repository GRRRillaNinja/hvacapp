...
// ── Keep keyboard open on app-switch ──────────────
let _lastInput = null;
document.addEventListener('focusin', e => {
    if (e.target.matches('input, textarea, select')) _lastInput = e.target;
});

(function () {
    const banner = document.createElement('div');
    banner.id = 'resume-banner';
    banner.textContent = 'Tap to resume typing';
    ...
    function showBanner() {
        ...
        // Ignore taps for 600ms — prevents the return-to-app gesture from instantly dismissing it
        setTimeout(() => { bannerReady = true; }, 600);
        bannerTimer = setTimeout(() => { banner.style.display = 'none'; }, 8000);
    }
    ...
    banner.addEventListener('click', () => {
        ...
        clearTimeout(bannerTimer);
        if (_lastInput) { try { _lastInput.focus(); } catch (_) {} }
    });

    let returned = false;
    function onReturn() {
        if (!_lastInput || returned) return;
        returned = true;
        // Immediate synchronous focus — Firefox may open keyboard here
        try { _lastInput.focus(); } catch (_) {}
        // Show banner after 800ms as fallback if keyboard didn't open
